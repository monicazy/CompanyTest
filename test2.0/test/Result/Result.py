...
from Method.TestMethod import TestApi
from Cookie.Cookies import cookie
...
class TestResult:
    ...
    def test_result(self):
        ...
        for i in range(0, self.row - 1):
            try:
                if self.token[i] == 0:
                    api = TestApi(self.method[i], self.url[i], self.data[i].encode('utf-8')).test_api
                    if api.status_code == 200:
                        text_result.append(api.json())
                    else:
                        text_result.append({"status": api.status_code, "message": "HTTP状态码错误，接口请求失败"})
                elif self.token[i] == 1:
                    api = TestApi(self.method[i], self.url[i], self.data[i].encode('utf-8'), None, cookie).test_api
                    if api.status_code == 200:
                        text_result.append(api.json())
                    else:
                        text_result.append({"status": api.status_code, "message": "HTTP状态码错误，接口请求失败"})
