...
class ExcelForHttpProcess(object):
    ...
    def get_case_list(self, filePath, case_line_list=None, startLine=0, endLine=0):
        """Excel测试用例表格处理
        处理传入的Excel表格，转换成测试用例List
        获取excel数据并返回一个list，list包含N个dict，每个tdic保存用例Excel中的各个值。规则如下：
        ...
        if len(case_line_list) == 0 and startLine > 0 and endLine > 0:
            ...
            raise ValueError(u'ExcelForHttpProcess.py: get_case_list: 传入参数错误')
    ...
    def __get_by_xlrd(self, filePath, case_line_list, by_index=0):
        ...
        try:
            ...
            raise IOError(
                u"ExcelForHttpProcess.py: __get_by_xlrd: data.sheets()[by_index]: by_index长度超过excel中的长度")
        nrows = table.nrows
        ...
        if json.dumps(table.row_values(0), ensure_ascii=False) == json.dumps(
                ["用例编号", "名称", "描述", "数据初始化", "数据恢复", "HTTP请求头信息", "接口", "参数", "预期结果", "变量配置"],
                ensure_ascii=False):
            ...
            for row_num in case_line_list:
                ...
                tmpCase = {'case_id': row_values[0].replace("\n", "").strip(),
                           'name': row_values[1],
                           'desc': row_values[2],
                           'data_init': row_values[3],
                           'data_recover': row_values[4],
                           'header': row_values[5],
                           'url': row_values[6].replace("\n", "").strip(),
                           'params': row_values[7].replace("\n", "").strip(),
                           'expect': row_values[8].strip(),
                           'varsconf': str(row_values[9]).replace("\n", "").strip()}
    ...
    def __get_by_value(self, filePath, startLine, endLine, by_index=0):
        ...
        if json.dumps(table.row_values(0), encoding='UTF-8', ensure_ascii=False) == json.dumps(
            ...
            if 1 < startLine < endLine < nrows:
                ...
                raise ValueError(
                    u'ExcelForHttpProcess.py: __get_by_value: 传入数据的起始行大于Excel文件最大行数或起始行小于1!'.encode('utf8'))
            return case_list
        else:
...
if __name__ == "__main__":
    ...
    try:
        tables = excelProcess.get_case_list('e:/ch-3.7.xlsx')
        for row in tables:
